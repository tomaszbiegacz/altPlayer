...
TEST(pcm__pcm_spec, setup_minimum) {
  ...
  pcm_spec_set_bytes_per_sample(builder, 2);
...
TEST(pcm__pcm_spec, setup_full) {
  ...
  pcm_spec_set_bytes_per_sample(builder, 3);
  pcm_spec_set_samples_per_sec(builder, 4);
  pcm_spec_set_frames_count(builder, 5);
  ...
  EXPECT_EQ(5, pcm_spec_get_frames_count(spec));
